...
#               Wayland‑Fallback, Lupenwerkzeug, Annotationen, Pixelierung,
#               Undo/Redo, Farbwahl, PNG‑Export und "Öffnen mit..." Funktion
#               NEU: Ganzer Bildschirm, Aktives Fenster, Bereich auswählen
#
...
# Der folgende Import ist für die Cairo‑Brücke notwendig; falls er fehlschlägt,
# muss das Paket python3-gi-cairo installiert werden.
# Wir rufen require_foreign nicht auf, da es in manchen Umgebungen Probleme gibt.
import cairo
...
# ===================== Konfiguration für "Öffnen mit..." =====================

CONFIG_FILE = os.path.expanduser("~/.config/guideos-screenshot-editor.json")
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, 'r') as f:
                config = json.load(f)
                return config.get("default_app", None), config.get("default_app_name", None)
    except:
        pass
    return None, None

def save_default_app(app_path, app_name):
    try:
        os.makedirs(os.path.dirname(CONFIG_FILE), exist_ok=True)
        config = {"default_app": app_path, "default_app_name": app_name}
...
def get_app_icon(app_name, app_path):
    """Versucht das Icon einer Anwendung zu finden"""
    icon_names = []
    
    possible_names = [
        app_name.lower(),
        app_path.split('/')[-1].lower(),
    ...
    for name in possible_names:
        if name in icon_map:
            icon_names.append(icon_map[name])
        icon_names.append(name)
    
    icon_names.extend(["applications-graphics", "image-x-generic", "image-viewer"])
    
    for icon_name in icon_names:
        try:
            icon_theme = Gtk.IconTheme.get_default()
            if icon_theme.has_icon(icon_name):
                return icon_theme.load_icon(icon_name, 32, 0)
        except:
            pass
    
    desktop_paths = [
        f"/usr/share/applications/{app_name.lower()}.desktop",
        f"/usr/share/applications/{app_path.split('/')[-1]}.desktop",
        f"/usr/local/share/applications/{app_name.lower()}.desktop",
    ]
...
def find_installed_image_apps():
    known_apps = [
        ("GIMP", "gimp"), ("Krita", "krita"), ("Pinta", "pinta"),
        ("Inkscape", "inkscape"), ("Shotwell", "shotwell"),
        ("GNOME Bildbetrachter", "eog"), ("LibreOffice Draw", "libreoffice"),
        ("KolourPaint", "kolourpaint"), ("MyPaint", "mypaint"),
        ("Firefox", "firefox"), ("Chrome", "google-chrome"),
        ("Chromium", "chromium"), ("ImageMagick", "display"),
    ]
    installed = []
    system_paths = os.environ.get('PATH', '').split(':')
    for app_name, bin_name in known_apps:
        for path in system_paths:
            full_path = os.path.join(path, bin_name)
            if os.path.exists(full_path) and os.access(full_path, os.X_OK):
                icon = get_app_icon(app_name, full_path)
                installed.append((app_name, full_path, icon))
                break
    return installed
...
class ScreenshotEngine:
    ...
    def capture_fullscreen():
        """Ganzen Bildschirm aufnehmen (alle Monitore zusammen)"""
        root = Gdk.get_default_root_window()
        if not root:
            return None
        w = root.get_width()
    ...
    def capture_active_window():
        ...
        Strategie: xdotool → wmctrl → Fallback ganzer Bildschirm
        """
        # Methode 1: xdotool (zuverlässigste Methode)
        try:
            r = subprocess.run(["xdotool", "getactivewindow", "getwindowgeometry", "--shell"],
            ...
            print(f"xdotool nicht verfügbar: {e}")

        # Methode 2: wmctrl
        try:
            r = subprocess.run(["wmctrl", "-lG"], capture_output=True, text=True, timeout=2)
            r2 = subprocess.run(["xprop", "-root", "_NET_ACTIVE_WINDOW"],
                                 capture_output=True, text=True, timeout=2)
                    ...
                    for line in r.stdout.strip().splitlines():
                        ...
                        if len(cols) >= 7:
                            try:
                                wid = int(cols[0], 16)
                                if wid == win_id_dec:
                                    x = int(cols[2]); y = int(cols[3])
                                    w = int(cols[4]); h = int(cols[5])
                                    if w > 0 and h > 0:
                                        root = Gdk.get_default_root_window()
                                        pb = Gdk.pixbuf_get_from_window(root, x, y, w, h)
                                        if pb:
            ...
            print(f"wmctrl nicht verfügbar: {e}")

        # Fallback: ganzer Bildschirm
        print("Aktives Fenster nicht ermittelbar, Fallback auf ganzen Bildschirm")
        return ScreenshotEngine.capture_fullscreen()
...
class EditorWindow(Gtk.Window):
    def __init__(self, image_pixbuf, request_mode=False):
        ...
        # Für "Öffnen mit..."
        self.installed_apps = find_installed_image_apps()
        self.default_app_path, self.default_app_name = load_default_app()

        # ==================== CSS MIT GTK4/ADWAITA STYLE FÜR AKTIVE SCHALTER ====================
        css = f"""
        .tool-button {{
        ...
        /* GTK4/Adwaita Style für Combobox - aktiviert mit Akzentfarbe GEFÜLLT */
        .combobox {{
            background-color: {self.theme_colors['button_bg']};
            color: {self.theme_colors['button_text']};
            border: 1px solid {self.theme_colors['border']};
        ...
        /* AKTIVIERTER COMBOBOX - VOLL MIT AKZENTFARBE GEFÜLLT (wie GTK4/Adwaita) */
        .combobox:active, 
        .combobox:checked,
        .combo-active {{
            background-color: {self.theme_colors['accent']};
        ...
        /* GTK4/Adwaita Style für ToggleButton */
        .toggle-button {{
            background-color: {self.theme_colors['button_bg']};
            color: {self.theme_colors['button_text']};
            border: 1px solid {self.theme_colors['border']};
        ...
        GtkLabel {{ 
            color: {self.theme_colors['fg_primary']}; 
        }}
        GtkHeaderBar {{ 
        ...
        self.pixel_sizes      = [4, 6, 8, 10, 12, 16, 20, 24, 32]
        ...
        # "Öffnen mit..." Button
        open_with_btn = Gtk.Button.new_with_label("Öffnen mit...")
        open_with_btn.get_style_context().add_class("suggested-action")
        open_with_btn.set_tooltip_text("In einem installierten Programm öffnen")
        open_with_btn.connect("clicked", self.on_open_with_clicked)
        header_bar.pack_end(open_with_btn)
        ...
        self.pixelsize_combo.set_active(3)
    ...
    # ------------------------------------------------------------------ werkzeug-handler mit Akzentfarbe
    ...
    def on_linewidth_changed(self, combo):
        ...
        self._activate_tool_from_combo(self.linewidth_combo, self.line_btn, Tool.LINE)

    def on_textsize_changed(self, combo):
        idx = combo.get_active()
        if 0 <= idx < len(self.font_sizes):
            self.current_text_size = self.font_sizes[idx]
        self._activate_tool_from_combo(self.textsize_combo, self.text_btn, Tool.TEXT)

    def on_magnifier_changed(self, combo):
        idx = combo.get_active()
        if 0 <= idx < len(self.magnifier_scales):
            self.magnifier_scale = self.magnifier_scales[idx]
        self._activate_tool_from_combo(self.magnifier_combo, self.magnifier_btn, Tool.MAGNIFIER)

    def on_pixelsize_changed(self, combo):
        idx = combo.get_active()
        if 0 <= idx < len(self.pixel_sizes):
            self.pixel_size = self.pixel_sizes[idx]
        self._activate_tool_from_combo(self.pixelsize_combo, self.pixelate_btn, Tool.PIXELATE)

    def _activate_tool_from_combo(self, combo, tool_btn, tool_id):
        # Nur wenn das Combo aktiviert ist (also wenn das entsprechende Tool aktiv ist)
        if combo.get_sensitive() and not tool_btn.get_active():
            tool_btn.set_active(True)
    ...
    # ------------------------------------------------------------------ "ÖFFNEN MIT..." FUNKTION MIT ICONS

    def save_current_image(self):
        try:
            fd, temp_path = tempfile.mkstemp(suffix='.png', prefix='screenshot_')
            os.close(fd)
            self.render_to_surface().write_to_png(temp_path)
            return temp_path
        except Exception as e:
            print(f"Fehler beim temporären Speichern: {e}")
            return None

    def on_open_with_clicked(self, btn):
        ...
        if self.installed_apps:
            title = Gtk.Label()
            title.set_markup(f"<b>📂 Installierte Programme ({len(self.installed_apps)})</b>")
            main_box.pack_start(title, False, False, 5)
        else:
            title = Gtk.Label()
            title.set_markup("<b>📂 Programm auswählen</b>")
            main_box.pack_start(title, False, False, 5)
        ...
        apps_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=3)

        if self.installed_apps:
            for app_name, app_path, icon in sorted(self.installed_apps, key=lambda x: x[0]):
                btn_app = Gtk.Button()
                btn_app.set_size_request(220, 40)
                btn_app.set_halign(Gtk.Align.CENTER)
                ...
                btn_app.add(hbox)
                btn_app.set_tooltip_text(f"Startbefehl: {app_path}")
                btn_app.connect("clicked", self.on_open_with_app, app_path, app_name, popover)
                apps_box.pack_start(btn_app, False, False, 2)
        else:
            info_label = Gtk.Label()
            info_label.set_markup("<i>Keine Programme gefunden.\nBitte manuell eingeben:</i>")
            info_label.set_margin_bottom(10)
            apps_box.pack_start(info_label, False, False, 5)

            entry = Gtk.Entry()
            entry.set_placeholder_text("z.B. gimp, krita, /usr/bin/gimp")
            entry.set_size_request(220, 35)
            apps_box.pack_start(entry, False, False, 5)

            run_btn = Gtk.Button.new_with_label("Starten")
            run_btn.get_style_context().add_class("suggested-action")
            run_btn.connect("clicked", self.on_custom_open, entry, popover)
            apps_box.pack_start(run_btn, False, False, 5)

        scrolled.add(apps_box)
        main_box.pack_start(scrolled, True, True, 0)

        if self.default_app_name:
            default_label = Gtk.Label()
            default_label.set_markup(f"<i>Zuletzt verwendet: {self.default_app_name}</i>")
            default_label.set_margin_top(5)
            main_box.pack_start(default_label, False, False, 0)

        popover.add(main_box)
        popover.show_all()
        popover.popup()

    def on_open_with_app(self, btn, command, app_name, popover):
        popover.hide()
        temp_file = self.save_current_image()
        if not temp_file:
            ...
            error_dialog.format_secondary_text("Das Bild konnte nicht temporär gespeichert werden.")
            error_dialog.run()
            error_dialog.destroy()
            return

        save_default_app(command, app_name)
        self.default_app_path = command
        self.default_app_name = app_name

        try:
            subprocess.Popen([command, temp_file])
        except Exception as e:
            error_dialog = Gtk.MessageDialog(
                transient_for=self, flags=0,
                message_type=Gtk.MessageType.ERROR,
                buttons=Gtk.ButtonsType.OK,
                text="Fehler beim Öffnen"
            )
            error_dialog.format_secondary_text(f"{str(e)}")
            error_dialog.run()
            error_dialog.destroy()
            try:
                os.unlink(temp_file)
    ...
    def on_custom_open(self, btn, entry, popover):
        ...
        try:
            ...
            subprocess.Popen(cmd_parts + [temp_file])
            save_default_app(command, command)
        except Exception as e:
            error_dialog = Gtk.MessageDialog(
                transient_for=self, flags=0,
                message_type=Gtk.MessageType.ERROR,
    ...
    def show_info(self):
        ...
        dlg.format_secondary_text(
            ...
            "📂 ÖFFNEN MIT...:\n"
            "• Bild direkt in GIMP, Krita, etc. öffnen\n"
            "• Automatische Erkennung installierter Programme\n"
            "• MIT ICONS für jedes Programm!\n\n"
            "Tastenkürzel:\n"
...
# ===================== Start-Fenster (mit drei Optionen) =====================
...
class StartWindow(Gtk.Window):
    def __init__(self):
        ...
        .start-btn:hover {{ background-color: {theme_colors['button_hover']}; }}
        ...
        hint_label.set_markup("<b>Was möchtest du aufnehmen?</b>")
        hint_label.set_margin_top(5)
        main_box.pack_start(hint_label, False, False, 0)

        # ---- Button: Bereich auswählen ----
        btn_area = Gtk.Button()
        btn_area.set_size_request(-1, 52)
        btn_area.get_style_context().add_class("suggested-action")
        btn_area.set_tooltip_text("Mit der Maus einen beliebigen Bereich aufziehen")
        hbox_area = Gtk.HBox(spacing=10)
        hbox_area.set_margin_start(12); hbox_area.set_margin_end(12)
        img_area = Gtk.Image.new_from_icon_name("view-fullscreen", Gtk.IconSize.LARGE_TOOLBAR)
        lbl_area = Gtk.Label()
        lbl_area.set_markup("<b>Bereich auswählen</b>\n<small>Mit der Maus aufziehen</small>")
        lbl_area.set_halign(Gtk.Align.START)
        hbox_area.pack_start(img_area, False, False, 0)
        hbox_area.pack_start(lbl_area, True, True, 0)
        btn_area.add(hbox_area)
        btn_area.connect("clicked", self.on_area_clicked)
        main_box.pack_start(btn_area, False, False, 0)

        # ---- Button: Ganzer Bildschirm ----
        btn_full = Gtk.Button()
        btn_full.set_size_request(-1, 52)
        btn_full.get_style_context().add_class("start-btn")
        btn_full.set_tooltip_text("Den gesamten Bildschirm aufnehmen")
        hbox_full = Gtk.HBox(spacing=10)
        hbox_full.set_margin_start(12); hbox_full.set_margin_end(12)
        img_full = Gtk.Image.new_from_icon_name("video-display", Gtk.IconSize.LARGE_TOOLBAR)
        lbl_full = Gtk.Label()
        lbl_full.set_markup("<b>Ganzer Bildschirm</b>\n<small>Alles aufnehmen</small>")
        lbl_full.set_halign(Gtk.Align.START)
        hbox_full.pack_start(img_full, False, False, 0)
        hbox_full.pack_start(lbl_full, True, True, 0)
        btn_full.add(hbox_full)
        btn_full.connect("clicked", self.on_fullscreen_clicked)
        main_box.pack_start(btn_full, False, False, 0)

        # ---- Button: Aktives Fenster ----
        btn_win = Gtk.Button()
        btn_win.set_size_request(-1, 52)
        btn_win.get_style_context().add_class("start-btn")
        btn_win.set_tooltip_text("Das gerade aktive Programmfenster aufnehmen")
        hbox_win = Gtk.HBox(spacing=10)
        hbox_win.set_margin_start(12); hbox_win.set_margin_end(12)
        img_win = Gtk.Image.new_from_icon_name("window", Gtk.IconSize.LARGE_TOOLBAR)
        lbl_win = Gtk.Label()
        lbl_win.set_markup("<b>Aktives Fenster</b>\n<small>Das aktuelle Fenster aufnehmen</small>")
        lbl_win.set_halign(Gtk.Align.START)
        hbox_win.pack_start(img_win, False, False, 0)
        hbox_win.pack_start(lbl_win, True, True, 0)
        btn_win.add(hbox_win)
        btn_win.connect("clicked", self.on_window_clicked)
        main_box.pack_start(btn_win, False, False, 0)

        # ---- Hinweis ----
        zoom_hint = Gtk.Label()
        zoom_hint.set_markup(
            f"<span size='small' style='italic'>"
            f"Tipp: Strg + Mausrad zum Zoomen im Editor  "
            f"│  Theme: {'Dunkel' if theme_mode == 'dark' else 'Hell'}"
            f"</span>")
        zoom_hint.set_margin_top(4)
    ...
    def on_fullscreen_clicked(self, btn):
        self.hide()
        while Gtk.events_pending(): Gtk.main_iteration_do(False)
        GLib.timeout_add(300, self.capture_fullscreen_delayed)

    def capture_fullscreen_delayed(self):
        pb = ScreenshotEngine.capture_fullscreen()
        self.open_editor_with_pixbuf(pb)
        return False

    # --- Aktives Fenster ---
    def on_window_clicked(self, btn):
        self.hide()
        while Gtk.events_pending(): Gtk.main_iteration_do(False)
        GLib.timeout_add(500, self.capture_active_window_delayed)

    def capture_active_window_delayed(self):
        pb = ScreenshotEngine.capture_active_window()
        self.open_editor_with_pixbuf(pb)
        return False
